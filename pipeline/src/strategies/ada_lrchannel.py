...
def main(inputMessage: dict[str, Any]) -> None:
	...
	dataFrame = dataFrame.with_columns(
		pl.when(
			(pl.col('close') > pl.col('signalUpLine')) & (pl.col('signalUpLine') > pl.col('close').shift(1)) &
		...
		pl.when(
			(pl.col('close') > pl.col('signalDownLine')) & (pl.col('signalDownLine') > pl.col('close').shift(1)) &
			(pl.col('close') < pl.col('trendMoving')) &
			(maxMulti > pl.col('volMulti')) & (pl.col('volMulti') > minMulti)
		).then(pl.lit(-1))
		.when(
