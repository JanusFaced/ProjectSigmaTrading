...
def algorithm(
	...
	fast_curve = simple_linear_regression(
		closeVector=dataFrame['close'].to_numpy(),
		baseWindow=int(0.5*signalWindow),
	)

	slow_curve = simple_linear_regression(
		closeVector=dataFrame['close'].to_numpy(),
		baseWindow=int(1.5*signalWindow),
	)

	dataFrame = dataFrame.with_columns([
		pl.lit(leverage).alias('leverage'),
		(pl.col('high')/pl.col('low') - 1).rolling_mean(window_size=trendWindow).alias('ATR'),
		pl.Series('fast_curve', fast_curve),
		pl.Series('slow_curve', slow_curve),
		...
		pl.when(
			(pl.col('fast_curve') > pl.col('slow_curve')) & (pl.col('slow_curve') > pl.col('fast_curve').shift(1)) &
