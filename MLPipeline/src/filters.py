...
def sort_timeframes(timeframes):
	...
	return sorted(timeframes, key=lambda x: priority.get(str(x), 9999))

def makeStats(tableBacktest: dict) -> None:
	...
	tableBacktest = newTableBacktest

	dataframe = pd.DataFrame(tableBacktest)

	split_parts = dataframe['strategy'].str.rsplit('_', n=4, expand=True)

	dataframe['strategy_name'] = split_parts[0]  # Все что до последних 4 частей
	...
	for metric_name in ['year_profit', 'max_drawdown', 'sharp']:
		pivot = pd.pivot_table(
			dataframe,
			values=metric_name,
			index='strategy_name',      # Строки - названия стратегий
			columns='timeframe',        # Столбцы - таймфреймы
			aggfunc='mean',             # Среднее значение
			margins=True,               # Добавляем строки/столбцы с итогами
			margins_name='final_mean'
		)

		cols = [col for col in pivot.columns if col != 'final_mean']
		sorted_cols = sort_timeframes(cols)
		...
		sns.heatmap(
			...
			square=False,         # Не делать ячейки квадратными
		...
		ax.set_xlabel('Таймфрейм', fontsize=12)
		ax.set_ylabel('Стратегия', fontsize=12)
		plt.xticks(rotation=45, ha='right')
		plt.yticks(rotation=0)
		plt.tight_layout()
		fileName: str = f'{output_dir}/stats_{metric_name}.png'
		plt.savefig(fileName, dpi=300, bbox_inches='tight', facecolor='white')
		plt.close()

		logger.info(f"pivot {metric_name} is save to {fileName}")
