...
def main(inputMessage: dict[str, Any], dataFrame: pd.DataFrame) -> pd.DataFrame:
	...
	signal_window_short = 10
	signal_window_long = 20
	trend_window = 200
	...
	dataFrame['signal_window_short'] = (signal_window_short*base_volativity/dataFrame['volativity']).fillna(signal_window_short).astype(np.int64)
	dataFrame['adaptive_moving_short'] = adaptive_moving(closeVector=dataFrame['close'].values, windowVector=dataFrame['signal_window_short'].values)

	dataFrame['signal_window_long'] = (signal_window_long*base_volativity/dataFrame['volativity']).fillna(signal_window_long).astype(np.int64)
	dataFrame['adaptive_moving_long'] = adaptive_moving(closeVector=dataFrame['close'].values, windowVector=dataFrame['signal_window_long'].values)
	...
	dataFrame['long_signal'] = np.select(
		[
			(dataFrame['adaptive_moving_short'] > dataFrame['adaptive_moving_long']) & (dataFrame['adaptive_roc'] > 0),
			(dataFrame['adaptive_moving_short'] < dataFrame['adaptive_moving_long']) & (dataFrame['adaptive_roc'] > 0)
		],
		[
			-1,
