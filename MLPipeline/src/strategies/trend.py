...
def main(inputMessage: dict[str, Any], dataFrame: pd.DataFrame) -> pd.DataFrame:
	...
	dataFrame['signal_trend'] = adaptive_roc(closeVector=dataFrame['close'].values, windowVector=dataFrame['signal_window'].values)
	...
	dataFrame['long_signal'] = np.select(
		[
			(dataFrame['signal_trend'] > 0) & (dataFrame['trend'] > 0),
			(dataFrame['signal_trend'] < 0) & (dataFrame['trend'] > 0)
		],
		[
			-1,
